...
class Validator:
    ...
    def validate (value):
        if not isinstance(value, str):
            raise TypeError(f'Значение должно быть текстом')
        if not value.isalpha():
            raise TypeError(f'Значение должно содержать только буквы')
...
class Student:
    ...
    def __init__(self, name: str, second_name: str, lessons: Path):
        self.name = name
...
if __name__ == '__main__':
    std = Student('Alexey', 'Kornev', Path('lessons.csv'))
    print(std)
    std.new_estimate("русский язык", 3)
    std.new_estimate("сопротивление материалов", 5)
    std.new_estimate("физика", 2)
    std.new_estimate("физика", 4)
    std.new_estimate("математика", 5)
    std.new_estimate("русский язык", 2)
    std.new_estimate("сопротивление материалов", 3)
    std.new_estimate("физика", 5)
    std.new_estimate("физика", 2)
    std.new_estimate("математика", 4)
    std.new_estimate("математика", 68, "test")
    std.new_estimate("математика", 99, "test")
    std.new_estimate("русский язык", 29, "test")
    std.new_estimate("русский язык", 100, "test")
    std.new_estimate("концепции современного естествознания", 18, "test")
    std.new_estimate("концепции современного естествознания", 74, "test")
    std.new_estimate("сопротивление материалов", 39, "test")
    std.new_estimate("сопротивление материалов", 89, "test")
    print(std)